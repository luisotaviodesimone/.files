...
alias gb-clean="git fetch -p && for branch in $(if [[ -d .git ]]; then git for-each-ref --format '%(refname) %(upstream:track)' refs/heads | awk '$2 == "[gone]" {sub("refs/heads/", "", $1); print $1}'; fi); do git branch -D $branch; done"
